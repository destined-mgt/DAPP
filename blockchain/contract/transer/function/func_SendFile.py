...
def SendFile(swarmId, reciver):
    nonce = conn.eth.getTransactionCount(addr)
    try:
        gas = contract.functions.SendFile(swarmId, reciver).estimateGas()
        contract.functions.SendFile(swarmId, reciver).call()

        tx = contract.functions.SendFile(swarmId, reciver).buildTransaction({
